...
Example metrics endpoint call result (substitute `<port>` with the value of `clientInfo.port` you configured):
```
$ curl localhost:<port>/metrics
# TYPE connected_peers_count gauge
connected_peers_count 108 1623235129569

# TYPE connected_wellknown_peers_count gauge
connected_wellknown_peers_count 10 1623235129569

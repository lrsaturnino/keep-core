...
following node-local roster metrics are recorded by the node-local cutover peer
roster (`pkg/protocol/participation`). They deduplicate every post-cutover
legacy-wire sighting down to the normalized operator address; they never carry
operator, session, or peer labels.

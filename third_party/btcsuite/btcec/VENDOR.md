...
(comment reflow only — required by this repository's CI formatting
check). No code changes whatsoever; see the verification recipe below,
which reproduces the exact expected content from upstream.
...
- first-party key-handling code (`pkg/crypto/ephemeral`,
  ...
  `pkg/internal/pbutils`), which relies on the v1 `btcec.PublicKey` /
...
their GHSA equivalents). Serving only `btcec` from this directory lets
the main btcd module track a current, fully patched release while the
btcec consumers keep compiling against byte-identical crypto sources.
